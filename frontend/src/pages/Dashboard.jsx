...
export default function Dashboard() {
    ...
    const scriptsConnectedPieConfig = {
        data: {
    ...
    const currentInstances = sessions.slice(0, 10);
                ...
                <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_15px_35px_rgba(15,23,42,0.06)]">
                    ...
                    <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="mb-3 flex items-center justify-between">
                            <h4 className="text-sm font-semibold text-slate-800">Current instances connected</h4>
                            <span className="text-xs text-slate-500">{currentInstances.length} visible</span>
                        </div>
                        ...
                        {currentInstances.length > 0 ? (
                                ...
                                <table className="min-w-full text-left text-sm">
                                    <thead>
                                        <tr className="text-xs uppercase tracking-wide text-slate-500">
                                            ...
                                            <th className="pb-2 pr-3">Executor</th>
                                            <th className="pb-2 pr-3">Script</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {currentInstances.map((session) => (
                                            <tr key={session.id} className="border-t border-slate-200/70 text-slate-700">
                                                <td className="py-2 pr-3 font-medium">{session.roblox_user || '-'}</td>
                                                <td className="py-2 pr-3">{session.executor || 'Unknown'}</td>
                                                <td className="py-2 pr-3">{session.script_name || session.script_slug || '-'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <p className="text-sm text-slate-500">No live instances currently connected.</p>
                        )}
                    </div>
                </article>
